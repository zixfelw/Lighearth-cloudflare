...
// Version: 1.9.1 - ALERT ONCE FIX
// 
// v1.9.1 CHANGES:
// - FIX: Alert Once logic - mỗi ngưỡng chỉ báo 1 lần/ngày
// - FIX: Auto reset alerts khi sang ngày mới (00:00 VN)
// - FIX: Reset alerts khi thay đổi cài đặt ngưỡng
// - NEW: Lưu ngày alert để tracking chính xác hơn
//
// v1.9.0 FEATURES:
// - Ultra Short Deep Link (≤64 chars)
// - Short Format: add_DEVICEID_NNNNNN_bf_bl_pv_gr_ld_loc
// - Smart Thresholds với Alert Once
// - Weather Forecast
...
// ============================================
// 🔔 ALERT ONCE SYSTEM v1.9.1
// ============================================
// Key format: th_TYPE_chatId_deviceId
// Value format: JSON { date: "YYYY-MM-DD", threshold: value }
// 
// Logic:
// 1. Check if alert exists for today with same threshold
// 2. If different day -> allow alert (reset daily at midnight)
// 3. If same day but different threshold -> allow alert
// 4. If same day and same threshold -> SKIP (already alerted)

async function getThresholdAlert(env, type, chatId, deviceId) {
...
async function setThresholdAlert(env, type, chatId, deviceId, thresholdValue) {
  if (!env.BOT_KV) return false;
  const key = `th_${type}_${chatId}_${deviceId}`;
  const vnDate = getVietnamDate();
  const data = { date: vnDate, threshold: thresholdValue };
  try { 
    // TTL 48h để đảm bảo xóa sau 2 ngày
    await env.BOT_KV.put(key, JSON.stringify(data), { expirationTtl: 172800 }); 
    return true; 
  } catch (e) { return false; }
}

async function shouldSendThresholdAlert(env, type, chatId, deviceId, currentThreshold) {
  const alertData = await getThresholdAlert(env, type, chatId, deviceId);
  const vnDate = getVietnamDate();
  
  // Chưa có alert trước đó -> cho phép
  if (!alertData) return true;
  
  // Khác ngày -> cho phép (reset hàng ngày lúc 00:00)
  if (alertData.date !== vnDate) return true;
  
  // Cùng ngày nhưng khác ngưỡng -> cho phép (user đã thay đổi cài đặt)
  if (alertData.threshold !== currentThreshold) return true;
  
  // Cùng ngày + cùng ngưỡng -> KHÔNG cho phép (đã báo rồi)
  return false;
}

async function clearThresholdAlert(env, type, chatId, deviceId) {
  if (!env.BOT_KV) return false;
  const key = `th_${type}_${chatId}_${deviceId}`;
...
async function clearAllThresholdAlerts(env, chatId, deviceId) {
  ...
  for (const type of types) { await clearThresholdAlert(env, type, chatId, deviceId); }
}
...
async function processNotifications(env) {
  ...
  for (const userDevice of devicesData) {
    ...
    // ============================================
    // ⚙️ CUSTOM THRESHOLD ALERTS v1.9.1
    // Logic: shouldSendThresholdAlert kiểm tra:
    // 1. Chưa alert hôm nay -> cho phép
    // 2. Khác ngày (reset 00:00) -> cho phép  
    // 3. Cùng ngày nhưng khác ngưỡng (user đổi cài đặt) -> cho phép
    // 4. Cùng ngày + cùng ngưỡng -> SKIP (đã báo rồi)
    // ============================================
    
    // 🔋💚 PIN ĐẦY (Custom)
    if (thresholds.batteryFull < 100 && rt.batterySoc >= thresholds.batteryFull) {
      if (await shouldSendThresholdAlert(env, 'full', chatId, deviceId, thresholds.batteryFull)) {
        ...
        await setThresholdAlert(env, 'full', chatId, deviceId, thresholds.batteryFull);
      }
    }
    
    // 🪫🔴 PIN THẤP (Custom)
    if (thresholds.batteryLow > 0 && rt.batterySoc <= thresholds.batteryLow) {
      if (await shouldSendThresholdAlert(env, 'low', chatId, deviceId, thresholds.batteryLow)) {
        notifications.push({ chatId, message: `🪫🔴 *PIN THẤP*\n📱 \`${deviceId}\`\n\n${getBatteryIcon(rt.batterySoc)} Pin: *${rt.batterySoc}%*\n🎯 Ngưỡng: ${thresholds.batteryLow}%\n\n☀️ PV: *${rt.pvPower}W*\n🏠 Load: *${rt.loadPower}W*\n⚡ Grid: *${rt.gridPower}W* ${getGridIcon(haDevice.hasGridPower)}\n\n🕐 ${getVietnamTime()}` });
        await setThresholdAlert(env, 'low', chatId, deviceId, thresholds.batteryLow);
      }
    }
    
    // ☀️🎉 PV ĐẠT NGƯỠNG
    if (thresholds.pvDaily > 0 && de.pvDay >= thresholds.pvDaily) {
      if (await shouldSendThresholdAlert(env, 'pv', chatId, deviceId, thresholds.pvDaily)) {
        ...
        await setThresholdAlert(env, 'pv', chatId, deviceId, thresholds.pvDaily);
      }
    }
    
    // ⚡⚠️ EVN ĐẠT NGƯỠNG
    if (thresholds.gridUsage > 0 && de.gridDay >= thresholds.gridUsage) {
      if (await shouldSendThresholdAlert(env, 'grid', chatId, deviceId, thresholds.gridUsage)) {
        ...
        await setThresholdAlert(env, 'grid', chatId, deviceId, thresholds.gridUsage);
      }
    }
    
    // 🏠📈 TIÊU THỤ ĐẠT NGƯỠNG
    if (thresholds.loadDaily > 0 && de.loadDay >= thresholds.loadDaily) {
      if (await shouldSendThresholdAlert(env, 'load', chatId, deviceId, thresholds.loadDaily)) {
        ...
        await setThresholdAlert(env, 'load', chatId, deviceId, thresholds.loadDaily);
      }
    }
...
async function handleStart(chatId, text, env, devicesData) {
  ...
  // ============================================
  // NEW SHORT FORMAT v1.9.0: add_DEVICEID_NNNNNN_bf_bl_pv_gr_ld_loc
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/health') { 
      ...
      return jsonResponse({ status: 'ok', version: '1.9.1', features: ['Short Deep Link ≤64 chars', 'Web UI Sync', 'Smart Thresholds', 'Alert Once', 'Weather'], mode: 'Direct_HA', storage: hasKV ? 'KV_Persistent' : 'In-Memory', notifications: 'enabled', webAPI: 'enabled', users: count }); 
    }
    
    // Default HTML page
    return corsResponse(`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>LightEarth Bot v1.9.1</title><style>body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif;max-width:700px;margin:50px auto;padding:20px;background:#0f172a;color:#e2e8f0}h1{color:#22d3ee}h2{color:#a78bfa;border-bottom:1px solid #334155;padding-bottom:10px}ul{list-style:none;padding-left:0}li{padding:8px 0;border-bottom:1px solid #1e293b}a{color:#22d3ee;text-decoration:none}a:hover{text-decoration:underline}.badge{background:#059669;color:white;padding:3px 8px;border-radius:4px;font-size:12px;margin-right:5px}.new{background:#dc2626}.code{background:#1e293b;padding:8px 12px;border-radius:4px;font-family:monospace;font-size:13px;display:block;margin:10px 0;overflow-x:auto}</style></head><body><h1>🤖 LightEarth Bot v1.9.1</h1><p><span class="badge new">✨ FIX START_PARAM_TOO_LONG</span><span class="badge">🔗 Short Deep Link ≤64 chars</span></p><h2>🔗 Deep Link Format v1.9.0:</h2><p>NEW (44 chars):</p><code class="code">add_P250802210_111110_95_20_10_5_15_hcm</code><p>OLD v1.8.0 (75 chars) - TOO LONG:</p><code class="code">add_P250802210_mg_po_pr_lb_pe_hs_loc_TP_Ho_Chi_Minh_bf100_bl20_pv0_gr0_ld0</code><h2>📋 Format: add_DEVICEID_NNNNNN_bf_bl_pv_gr_ld_loc</h2><ul><li><strong>NNNNNN</strong>: 6 bits thông báo (1=bật, 0=tắt)</li><li>Bit 1: morningGreeting | Bit 2: powerOutage | Bit 3: powerRestored</li><li>Bit 4: lowBattery | Bit 5: pvEnded | Bit 6: hourlyStatus</li><li>VD: 111110 = bật 5 cái đầu, tắt hourlyStatus</li><li><strong>bf_bl_pv_gr_ld</strong>: ngưỡng (batteryFull, batteryLow, pvDaily, gridUsage, loadDaily)</li><li><strong>loc</strong>: mã vùng 2-4 ký tự (hcm, hn, dng, tn, bd, dn, la...)</li></ul><h2>📍 Mã vùng:</h2><ul><li>hcm - TP. Hồ Chí Minh | hn - Hà Nội | dng - Đà Nẵng | ct - Cần Thơ</li><li>bd - Bình Dương | tn - Tây Ninh | dn - Đồng Nai | dl - Đà Lạt | la - Long An</li></ul><h2>📱 Commands:</h2><ul><li>/start - 🚀 Bắt đầu + Deep Link</li><li>/help - 📋 Hướng dẫn</li><li>/add, /remove, /list - 📱 Quản lý thiết bị</li><li>/status, /check - 📊 Trạng thái</li><li>/settings - 🔔 Cài đặt thông báo</li><li>/thresholds - 🎯 Ngưỡng cảnh báo</li><li>/location - 📍 Vùng thời tiết</li></ul><h2>🔧 API & Debug:</h2><ul><li><a href="/health">/health</a> - Trạng thái hệ thống</li><li><a href="/kv-status">/kv-status</a> - Trạng thái KV</li><li><a href="/api/generate-deeplink?deviceId=P250802210&notifications=111110&bf=95&bl=20&pv=10&gr=5&ld=15&loc=hcm">/api/generate-deeplink</a> - Tạo short link</li><li><a href="/trigger-notifications">/trigger-notifications</a> - Test gửi thông báo</li></ul></body></html>`, { headers: { 'Content-Type': 'text/html; charset=utf-8' } });
  },
