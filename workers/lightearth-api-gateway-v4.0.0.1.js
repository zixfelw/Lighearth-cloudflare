/**
 * LightEarth API Gateway v4.0.0.1
 ...
 * 
 * v4.0.0.1 Changes:
 * - REMOVED: All IP rate limiting
 * - REMOVED: All device rate limiting
 * - REMOVED: Geographic blocking (China IPs)
 * - REMOVED: User-Agent blocking
 * - REMOVED: Origin validation (allow all)
 * - Fully open API for maximum reliability
 * - Only keep basic security headers
 * 
 * Security:
 * - Basic CORS headers
 * - Input sanitization for device IDs
 * - API key protection for /api/cloud/* endpoints only
 */
...
export default {
  async fetch(request, env, ctx) {
    ...
    // NO GEO BLOCKING - All countries allowed
    // NO USER-AGENT BLOCKING - All clients allowed
    ...
    if (path === '/' || path === '/health') {
      return new Response(JSON.stringify({
        status: 'ok',
        version: '4.0.0.1',
        rateLimit: 'DISABLED - No limits',
        geoBlock: 'DISABLED - All countries allowed',
        features: ['realtime', 'power-history', 'soc-history', 'temperature', 'device-info'],
        note: 'All rate limiting and blocking removed for maximum reliability'
      }), { headers });
    }
    ...
    // Cloud API Endpoints (Protected by API Key only)
    
    if (path === '/api/cloud/devices') {
      if (!PI_URL || !PI_TOKEN) {
        return new Response(JSON.stringify({ success: false, error: 'Service unavailable' }), { status: 503, headers });
      }
      // Only check API key, no other restrictions
    ...
    // LightEarth Public API Endpoints - NO RESTRICTIONS
...
async function fetchRealtimeDeviceData(piUrl, piToken, deviceId) {
  ...
  return {
    success: true,
    source: "CloudflareWorker_HA_v4.0.0.1",
    deviceData: {
