/**
 * Temperature-SOC-Power Worker v3.1
 * 
 * UPDATE v3.1:
...
async function getSOCHistory(deviceId, date, env) {
  ...
  data[0].forEach(item => {
    ...
    const minutes = Math.floor(vnTime.getUTCMinutes() / 5) * 5;
    const timeKey = `${hours.toString().padStart(2, '0')}:${minutes.toString().padStart(2, '0')}`;
    const soc = parseFloat(item.state);
    if (!isNaN(soc)) {
      timeSlots[timeKey] = { t: timeKey, soc: Math.round(soc) };
    }
  });
...
async function getTemperatureHistory(deviceId, date, env) {
  ...
  if (!data || data.length === 0 || !data[0] || data[0].length === 0) {
    return { success: true, deviceId, date, timeline: [], count: 0, message: 'No temperature data', version: VERSION };
  }
  ...
  const timeline = Object.values(timeSlots).sort((a, b) => a.t.localeCompare(b.t));
  return { success: true, deviceId, date, timeline, count: timeline.length, version: VERSION };
}
...
async function getYearlyStatisticsFromCurrentSensor(deviceId, env) {
  ...
  if (!pvYearSensor || !pvYearSensor.attributes) {
    return { success: false, deviceId: deviceUpper, year: getCurrentVietnamYear(), message: 'Chưa có dữ liệu', version: VERSION };
  }
...
async function getYearlyStatisticsFromHistory(deviceId, year, env) {
  ...
  try {
    ...
    if (!historyData || historyData.length === 0 || !historyData[0] || historyData[0].length === 0) {
      return { success: false, deviceId: deviceUpper, year, message: `Không có dữ liệu năm ${year} trong history`, version: VERSION };
    }
    
    // Find the record with matching year attribute
    const entityHistory = historyData[0];
    let bestRecord = null;
    
    for (const record of entityHistory) {
      if (record.attributes && record.attributes.year === year) {
        // Take the last record of that year (most complete data)
        if (!bestRecord || new Date(record.last_changed) > new Date(bestRecord.last_changed)) {
          bestRecord = record;
        }
      }
    }
    ...
    if (!bestRecord || !bestRecord.attributes) {
      // Fallback: try to find any record and check its year
      for (const record of entityHistory) {
    ...
    if (!bestRecord || !bestRecord.attributes) {
      return { success: false, deviceId: deviceUpper, year, message: `Không tìm thấy dữ liệu năm ${year}`, version: VERSION };
    }
    
    return extractYearlyDataFromAttributes(bestRecord.attributes, deviceUpper, 'history_api', year);
    ...
    <div class="content">
      ...
      <div class="summary">
        <div class="summary-item">
          <div class="label">☀️ PV Generation</div>
  ...
  <script>
    ...
    function copyData() {
      ...
      navigator.clipboard.writeText(jsonStr).then(() => {
        ...
        showStatus('✅ Đã copy JSON vào clipboard!', 'success');
        setTimeout(() => {
        ...
        showStatus('❌ Lỗi copy: ' + err.message, 'error');
      });
    }
