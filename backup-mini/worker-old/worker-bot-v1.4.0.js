...
// Version: 1.4.0 - Web Settings API
// 
...
async function addDevice(env, devicesData, chatId, deviceId) {
  ...
  devicesData.push({ deviceId: deviceId.toUpperCase(), chatId, addedAt: getVietnamTime(), location: "TP. Ho Chi Minh", notifications: { morningGreeting: true, powerOutage: true, powerRestored: true, lowBattery: true, pvEnded: true, hourlyStatus: false } });
...
async function processNotifications(env) {
  ...
  for (const userDevice of devicesData) {
    ...
    if (prefs.lowBattery && !prevState.isLowBattery && currentState.isLowBattery) {
      ...
      notifications.push({ chatId, message: `🚭 *CANH BAO PIN YEU - ${deviceId}*\n\n${getBatteryIcon(rt.batterySoc)} Pin: *${rt.batterySoc}%* - CAN SAC!\n☀️ PV: *${rt.pvPower}W*\n🏠 Load: *${rt.loadPower}W*\n⚡ Grid: *${rt.gridPower}W* ${haDevice.hasGridPower ? '🟢' : '🔴'}${tip}\n\n⏰ ${getVietnamTime()}` });
    }
...
async function handleHelp(chatId) {
  await sendTelegram(chatId, `🤖 *LightEarth Bot v1.3.0*\n💾 _KV Storage + Enhanced Weather_\n\n📋 *Quan ly thiet bi:*\n/add <ID> - Them thiet bi\n/remove <ID> - Xoa thiet bi\n/list - Danh sach thiet bi\n\n📊 *Trang thai:*\n/status - Trang thai tat ca\n/check <ID> - Kiem tra 1 thiet bi\n\n⚙️ *Cai dat:*\n/settings - Cai dat thong bao\n/location - Cai dat vung thoi tiet (tung thiet bi)\n\n🔔 *Thong bao tu dong:*\n• 🌅 Chao buoi sang + Du bao thoi tiet\n• ⚡ Mat dien luoi EVN + Goi y\n• ✅ Co dien lai (kem thoi gian mat)\n• 🚭 Pin yeu (<20%) + Canh bao\n• 🌇 Ket thuc ngay nang + Danh gia dem\n• ⏰ Bao cao moi gio (6h-21h) + Thoi tiet\n\n💡 Vi du: /check H250422132`);
}
...
async function handleUpdate(update, env) {
  ...
  switch (command) {
    case '/start': case '/help': await handleHelp(chatId); break;
  ...
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/api/update-settings' && request.method === 'POST') {
      try {
        ...
        const { deviceId, notifications, location } = body;
