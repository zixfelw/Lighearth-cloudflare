  ...
  for (const userDevice of devicesData) {
    ...
    if (prefs.morningGreeting && vnHour >= 6 && vnHour < 7) {
      ...
      if (notificationFlags[morningKey] !== vnDate) {
        ...
        let weatherInfo = '';
        if (weather) {
          weatherInfo = `\n\n${weather.icon} ${weather.currentTemp}°C | ${weather.humidity}% | ${weather.rainChance}% mưa`;
        }
...
async function handleStart(chatId, text, env, devicesData) {
  ...
  if (shortMatchV2) {
    ...
    const action = result.isNew ? '✅ *THÊM MỚI*' : '✅ *CẬP NHẬT*';
    await sendTelegram(chatId, `${action}\n\n📱 \`${deviceId.toUpperCase()}\`\n📍 ${location}\n\n🎯 Pin đầy: ${thresholds.batteryFull}% | Pin thấp: ${thresholds.batteryLow}%\n🔌 Điện áp: ${thresholds.batteryVoltHigh}V / ${thresholds.batteryVoltLow}V\n☀️ PV: ${thresholds.pvDaily}kWh | ⚡ EVN: ${thresholds.gridUsage}kWh | 🏠 Tải: ${thresholds.loadDaily}kWh\n\n⚙️ /settings /thresholds /location`, env);
    return result.devicesData;
  }
